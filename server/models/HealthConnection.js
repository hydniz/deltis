...
// nothing is read without an explicit opt-in. Every entry MUST have somewhere
// to go; advertising a type the sync endpoint silently drops would be worse
// than not offering it.
//
//   exercise  ExerciseSessionRecord → HealthActivity (a stored session, always
//             enriched with its distance/calories/steps/heart-rate)
//   weight    WeightRecord          → WeightLog (merged, see docs/HEALTH.md)
//   <metric>  every key in metricCatalog.HEALTH_METRICS → a MetricLog on the
//             matching user metric (body fat, resting HR, sleep, blood pressure,
//             steps, hydration, …). This is the generic measurement layer.
const { HEALTH_TYPE_KEYS } = require('../services/metricCatalog');
const SUPPORTED_TYPES = ['exercise', 'weight', ...HEALTH_TYPE_KEYS];
