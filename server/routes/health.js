...
function toActivityDoc(userId, record, deviceId) {
  ...
  const doc = {
    ...
    averageWatts: Number.isFinite(Number(record.avgWatts)) ? Number(record.avgWatts) : undefined,
    maxWatts: Number.isFinite(Number(record.maxWatts)) ? Number(record.maxWatts) : undefined,
    averageCadence: Number.isFinite(Number(record.avgCadence)) ? Number(record.avgCadence) : undefined,
    streams,
    raw: record,
